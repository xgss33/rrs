    ...
    std::span<const PlayerEntity> players)
{
    ...
    for (std::size_t target_player_index = 0; target_player_index < players.size(); ++target_player_index) {
        if (target_player_index == observer_player_index) {
            continue;
        }

        const auto& target_player = players[target_player_index];
        for (std::size_t target_ball_index = 0; target_ball_index < kMaxBallsPerPlayer; ++target_ball_index) {
            const auto target_ball_mask = static_cast<std::uint16_t>(1U << target_ball_index);
            if ((target_player.active_ball_mask & target_ball_mask) == 0) {
                continue;
            }
